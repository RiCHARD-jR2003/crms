...
<body>
    <div class="page">
        ...
        <div class="reference-box">
            ...
            <div class="ref-number">{{ $assessment->reference_number }}</div>
        </div>
        ...
        <div class="section">
            ...
            <div class="section-content">
                <table>
                    <tr>
                        <th>Full Name</th>
                        <td colspan="3">{{ $assessment->applicant_name }}</td>
                    </tr>
                    <tr>
                        <th>Email Address</th>
                        <td>{{ $assessment->applicant_email ?? 'N/A' }}</td>
                        <th>Contact Number</th>
                        <td>{{ $assessment->applicant_contact ?? 'N/A' }}</td>
                    </tr>
                    @if($application)
                    <tr>
                        <th>Address</th>
                        <td colspan="3">{{ $application->address }}, {{ $application->barangay }}, {{ $application->city }}</td>
                    </tr>
                    <tr>
                        <th>Birth Date</th>
                        <td>{{ $application->birthDate ? \Carbon\Carbon::parse($application->birthDate)->format('F d, Y') : 'N/A' }}</td>
                        <th>Gender</th>
            ...
            <div class="section-content">
                <table>
                    <tr>
                        ...
                        <td>{{ $assessment->assessment_date ? \Carbon\Carbon::parse($assessment->assessment_date)->format('F d, Y') : 'N/A' }}</td>
                        <th>Time Slot</th>
                        <td>{{ $timeSlots[$assessment->slot_number] ?? 'N/A' }}</td>
                    </tr>
                    <tr>
                        <th>Status</th>
                        <td colspan="3">
                            <span style="color: {{ $assessment->status === 'finalized' ? '#27AE60' : '#F39C12' }}; font-weight: bold;">
                                {{ strtoupper($assessment->status_label) }}
                            </span>
                        </td>
                    </tr>
                </table>
            ...
            <div class="section-content">
                <table>
                    <tr>
                        ...
                        <td>{{ $assessment->disability_type ?? 'N/A' }}</td>
                        <th>Severity</th>
                        <td>
                            @if($assessment->disability_severity)
                            <span class="severity-box severity-{{ $assessment->disability_severity }}">
                                {{ strtoupper($assessment->disability_severity) }}
                            </span>
                            @else
                            N/A
                            @endif
                    ...
                    <tr>
                        ...
                        <td>{{ $assessment->disability_cause ?? 'N/A' }}</td>
                        <th>Onset Date</th>
                        <td>{{ $assessment->disability_onset_date ? \Carbon\Carbon::parse($assessment->disability_onset_date)->format('F d, Y') : 'N/A' }}</td>
                    </tr>
                    <tr>
                        <th>Description</th>
                        <td colspan="3">{{ $assessment->disability_description ?? 'N/A' }}</td>
                    </tr>
                </table>
            </div>
        </div>
            ...
            <div class="section-content">
                <table>
                    <tr>
                        ...
                        <td>{{ $assessment->mobility_status ?? 'N/A' }}</td>
                    </tr>
                    <tr>
                        <th>Communication Ability</th>
                        <td>{{ $assessment->communication_ability ?? 'N/A' }}</td>
                    </tr>
                    <tr>
                        <th>Self-Care Ability</th>
                        <td>{{ $assessment->self_care_ability ?? 'N/A' }}</td>
                    </tr>
                    <tr>
                        <th>Learning Ability</th>
                        <td>{{ $assessment->learning_ability ?? 'N/A' }}</td>
                    </tr>
                    <tr>
                        <th>Functional Limitations</th>
                        <td>
                            @if($assessment->functional_limitations && count($assessment->functional_limitations) > 0)
                                @foreach($assessment->functional_limitations as $limitation)
                                <div class="list-item">{{ $limitation }}</div>
                                @endforeach
                            @else
                                N/A
                            @endif
            ...
            <div class="section-content">
                <table>
                    <tr>
                        ...
                        <td>{{ $assessment->attending_physician ?? 'N/A' }}</td>
                        <th>License No.</th>
                        <td>{{ $assessment->physician_license_no ?? 'N/A' }}</td>
                    </tr>
                    <tr>
                        <th>Medical Facility</th>
                        <td colspan="3">{{ $assessment->medical_facility ?? 'N/A' }}</td>
                    </tr>
                    <tr>
                        <th>Medical Findings</th>
                        <td colspan="3">{{ $assessment->medical_findings ?? 'N/A' }}</td>
                    </tr>
                    <tr>
                        <th>Recommendations</th>
                        <td colspan="3">{{ $assessment->recommendations ?? 'N/A' }}</td>
                    </tr>
                </table>
            </div>
        </div>
            ...
            <div class="section-content">
                <table>
                    <tr>
                        ...
                        <td>
                            @if($assessment->assistive_devices_current && count($assessment->assistive_devices_current) > 0)
                                {{ implode(', ', $assessment->assistive_devices_current) }}
                            @else
                                None
                            @endif
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($assessment->assistive_devices_needed && count($assessment->assistive_devices_needed) > 0)
                                {{ implode(', ', $assessment->assistive_devices_needed) }}
                            @else
                                None specified
                            @endif
                        </td>
        ...
        <!-- Notes -->
        @if($assessment->assessor_notes || $assessment->applicant_remarks)
        <div class="section">
            <div class="section-title">VII. ADDITIONAL NOTES</div>
            <div class="section-content">
                @if($assessment->assessor_notes)
                <div class="notes-box">
                    <h4>Assessor's Notes:</h4>
                    <p>{{ $assessment->assessor_notes }}</p>
                </div>
                @endif
                @if($assessment->applicant_remarks)
                <div class="notes-box" style="background: #E8F4FD; border-color: #0b87ac;">
                    <h4 style="color: #0b87ac;">Applicant's Remarks:</h4>
                    <p>{{ $assessment->applicant_remarks }}</p>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="signature-section">
            <div class="signature-box">
                <div class="signature-line">
                    <strong>{{ $assessment->applicant_name }}</strong><br>
                    <small>Applicant's Signature</small>
                </div>
            </div>
            <div class="signature-box">
                <div class="signature-line">
                    <strong>{{ $assessment->finalizer ? $assessment->finalizer->firstName . ' ' . $assessment->finalizer->lastName : '________________' }}</strong><br>
        ...
        <div class="footer">
            ...
            <p>Reference: {{ $assessment->reference_number }} | Generated: {{ $generatedAt }}</p>
            @if($assessment->finalized_at)
            <p>Finalized: {{ \Carbon\Carbon::parse($assessment->finalized_at)->format('F d, Y h:i A') }}</p>
            @endif
        </div>
    </div>
</body>
